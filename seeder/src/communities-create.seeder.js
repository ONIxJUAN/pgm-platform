...
(async () => {
  ...
  */
  const createCommunities = async (n = 20) => {
    for (let i=0; i < n;i++) {
      new Promise(resolve => setTimeout(resolve, 300 * i)).then(() => createCommunity({name: faker.lorem.word(), description: faker.lorem.paragraph(), type: 'public', authUserId: authUsers[generateValueBetweenMinAndMax(0, authUsers.length - 1)].id }));
    }
  };
